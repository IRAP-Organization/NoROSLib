...
ROS nodes (rospy/roscpp) and noros nodes register with. It implements the ROS
...
Run it directly:
    ...
    python3 -m noros.roscore                 # binds :11311, advertises this host
    python3 -m noros.roscore --port 11322
    ROS_MASTER_URI=http://myhost:11311 ROS_HOSTNAME=myhost python3 -m noros.roscore

Then point nodes at it with the matching ROS_MASTER_URI. Uses only the standard
library -- no ROS, no noros client code.
"""
import os
import sys
import socket
...
class RosMaster:
    ...
    def __init__(self, master_uri, quiet=False):
        ...
        self.params.set("/rosdistro", "noros\n")
        self.params.set("/rosversion", "noros 0.1.0\n")
...
class _Handler(SimpleXMLRPCRequestHandler):
    rpc_paths = ("/", "/RPC2")
    # HTTP/1.1 keep-alive: clients (rospy/roscpp/noros) reuse one connection for
