"""The noros node: registration with a real roscore, a slave XML-RPC server so
...
def set_master_uri(uri):
    """Set the ROS master URI noros registers with (e.g. 'http://10.0.0.5:11311').
...
def set_hostname(host):
    """Set the hostname/IP noros advertises to the master and peers (like
...
class _Publication:
    def __init__(self, node, topic, msg_class, latch):
        ...
        threading.Thread(target=self._accept_loop,
                         name="noros-pub-%s" % topic, daemon=True).start()
...
class _Subscription:
    ...
    def _connect_and_handshake(self, publisher_uri):
        ...
        recovery is the headline noros feature: a wrong/unknown md5 self-heals."""
        md5, type_name = self.md5, self.type_name
        for attempt in (1, 2):
...
class _ServiceServer:
    def __init__(self, node, name, srv_class, handler):
        ...
        threading.Thread(target=self._accept_loop,
                         name="noros-srv-%s" % name, daemon=True).start()
...
def init_node(name, master_uri=None, host=None):
    ...
    master_uri: the ROS master to register with. Precedence:
        this arg > noros.set_master_uri() > $ROS_MASTER_URI > http://127.0.0.1:11311.
    host: the address peers reach you at (advertised to master + nodes). Precedence:
        this arg > noros.set_hostname() > $ROS_IP > $ROS_HOSTNAME > auto-detected.
    """
    global _NODE
    if _NODE is None:
...
def get_node():
    if _NODE is None:
        raise RuntimeError("call noros.init_node(...) first")
    return _NODE
...
class Subscriber:
    ...
    receive raw bytes and auto-discover the type/md5 from the publisher.
        ...
        noros.Subscriber("/chatter", msg.String, cb)
        noros.Subscriber("/anything", None, cb)              # wildcard: raw bytes
        noros.Subscriber("/chatter", msg.String, cb, transport="udpros")  # UDP
    """
