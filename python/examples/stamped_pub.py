...
The .msg text just names `std_msgs/Header header` as its first field; irap_noroslib
...
See it (irap_noroslib side):   python3 stamped_sub.py
"""
import os
...
def main():
    ...
    irap_noroslib.init_node("stamped_pub")
    irap_noroslib.loginfo("SensorReading md5sum = %s" % SensorReading.md5sum())
    pub = irap_noroslib.Publisher("/sensor_reading", SensorReading)
    rate = irap_noroslib.Rate(5)
    seq = 0
    while not irap_noroslib.is_shutdown():
        m = SensorReading()
        m.header.seq = seq                     # ROS auto-fills this too
        m.header.stamp = irap_noroslib.now()           # (secs, nsecs)
        m.header.frame_id = "sensor_link"
        m.temperature = 20.0 + seq * 0.1
        m.humidity = 55.0
        m.sensor_id = "dht22-0"
        pub.publish(m)
        irap_noroslib.loginfo("published seq=%d temp=%.1f frame=%s"
