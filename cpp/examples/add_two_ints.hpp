// Defining your OWN service type in irap_noroslib C++.
//
// A irap_noroslib service type = static TYPE + MD5 (from `rossrv md5 <type>`) + nested
...
struct AddTwoInts {
  ...
  struct Request {
    ...
    int64_t a = 0, b = 0;
    std::vector<uint8_t> serialize() const { irap_noroslib::Writer w; w.i64(a); w.i64(b); return w.b; }
    static Request deserialize(const std::vector<uint8_t>& buf) {
      irap_noroslib::Reader r(buf); Request m; m.a = r.i64(); m.b = r.i64(); return m;
    }
  };
  struct Response {
    static constexpr const char* TYPE = "rospy_tutorials/AddTwoIntsResponse";
    int64_t sum = 0;
    std::vector<uint8_t> serialize() const { irap_noroslib::Writer w; w.i64(sum); return w.b; }
    static Response deserialize(const std::vector<uint8_t>& buf) {
      irap_noroslib::Reader r(buf); Response m; m.sum = r.i64(); return m;
    }
  };
};
